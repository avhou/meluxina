...
def process_model(model_input: ModelInput, database: str, triple_map: Dict[str, List[Triple]]):
    ...
    for prompt_type, prompt in model_input.prompts.items():
        progress_file = f"rq1_triples_{sanitized_model}_{prompt_type}.json"
...
def rq1(database: str, triple_file: str):
    with open(triple_file, "r") as f:
        ...
        print(f"read {len(triple_model.row_results)} triples from {triple_file}", flush=True)
        ...
        for row in triple_model.row_results:
            try:
                ...
                # print(f"found {len(triple_output.triples)} triples for {row.url}", flush=True)
        ...
        #     model_result = process_model(model, database, ttl_map)
