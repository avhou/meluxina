...
def process_model(model_input: ModelInput, database: str, triple_map: Dict[str, List[Triple]], triple_generation_model: str):
    ...
    for prompt_type, prompt in model_input.prompts.items():
        ...
        with sqlite3.connect(database) as conn:
            for row in conn.execute(f"select translated_text, disinformation, url from articles limit 1"):
                ...
                url = row[2]
                triples_for_url = triple_map.get(url, [])
...
def rq1(database: str, triple_file: str):
    with open(triple_file, "r") as f:
        ...
        print(f"read {len(triple_model.row_results)} triples from {triple_file}, triple_generation_model is {triple_generation_model}", flush=True)
        ...
        for model in model_inputs:
            print(f"processing model {model.model_name} and triple file {triple_file}", flush=True)
            model_result = process_model(model, database, triple_map, triple_generation_model)
            sanitized_model = sanitize_filename(model.model_name)
            with open(f"rq1_triples_{sanitized_model}_generated_by_{triple_generation_model}.json", "w") as f:
                f.write(model_result.model_dump_json(indent=2))

if __name__ == "__main__":
    if len(sys.argv) <= 2:
        raise RuntimeError("usage : rq1.py <combined-db.sqlite> <triple-file.json")
    rq1(sys.argv[1], sys.argv[2])
